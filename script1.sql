...
select r.path, h.name as homename, r.name as roomname, r.price, hb.startdate, hb.enddate, u.name, u.id, u.tel, hb.bsseq, hb.seq from tblHomeBook hb inner join tblRoom r on hb.rseq = r.seq inner join tblUser u on hb.id = u.id inner join tblHome h on r.hseq = h.seq where h.pid = 'partner';
    
select * from tblUser;

create or replace procedure usehomebook
is
begin
   update tblHomeBook set bsseq = 3;
    commit ;
end ;
...
select * from tblActivity a
inner join tblActivitybook ab on a.seq = ab.aseq 
inner join tblUser u on ab.id = u.id 
inner join tblBookList bl on ab.blseq = bl.seq 
inner join tblPayment p on bl.seq = p.blseq 
    where a.pid = 'partner';

select a.path, a.name, p.finalprice, ab.count, u.id, u.name as username, u.tel, ab.bsseq, ab.seq 
from tblActivity a
inner join tblActivitybook ab on a.seq = ab.aseq 
inner join tblUser u on ab.id = u.id 
inner join tblBookList bl on ab.blseq = bl.seq 
...
select a.path, a.name, p.finalprice, ab.count, u.id, u.name as username, u.tel, ab.bsseq, ab.seq from tblActivity a inner join tblactphoto ap on a.seq = ap.aseq inner join tblActivitybook ab on a.seq = ab.aseq inner join tblUser u on ab.id = u.id inner join tblBookList bl on ab.blseq = bl.seq inner join tblPayment p on bl.seq = p.blseq where a.pid = 'partner';
...
update tblActivitybook set bsseq = 1;
commit;

select cn.path, c.carname, p.finalprice, cb.startdate, cb.enddate, u.id, u.name, u.tel, cb.bsseq, cb.seq from tblCar c inner join tblcarname cn on c.carname = cn.seq inner join tblCarBook cb on cb.cseq = c.seq inner join tblUser u on cb.id = u.id inner join tblBookList bl on cb.bseq = bl.seq inner join tblPayment p on bl.seq = p.blseq where c.pid = 'partner';


select * from tblCar c inner join tblcarname cn on c.carname = cn.seq inner join tblCarBook cb on cb.cseq = c.seq inner join tblUser u on cb.id = u.id inner join tblBookList bl on cb.bseq = bl.seq ;

insert into tblBookList values (9, 'user');

update tblCarbook set bsseq = 1;

select * from tblPayment;

insert into tblPayment values (3, 9, 'n', 50000);
commit;


update tblHomebook set bsseq = 3 where enddate + 1 <= sysdate and bsseq = 2;
update tblHomebook set bsseq = 1;
commit;



select * from tblHomeBook;
select * from tblCarBook;
