...
insert into tblactivity values(4,'partner','2','오션월드 마지막 세일!','강원도 홍천군 서면 한치골길 262','2022-08-22',23000,'/tripnow/images/ocean.png','/tripnow/images/ocean.png','y','[유효기간]<br>- 유효기간 -<br>*골드시즌 : 구매 후 익일 ~ 8/21<br>*당일구매 당일사용불가*<br>[유의사항]<br>- 이용일 기준 구매 후 익일부터 사용 가능 합니다.');
...
insert into tblActPhoto values(10 ,5,'/tripnow/images/jeju.jpg');
insert into tblActPhoto values(11 ,7,'/tripnow/images/jeju.jpg');
insert into tblActPhoto values(12 ,8,'/tripnow/images/jeju.jpg');
insert into tblActPhoto values(13 ,9,'/tripnow/images/jeju.jpg');
insert into tblActPhoto values(14 ,11,'/tripnow/images/jeju.jpg');
...
insert into tblActivitybook values(3,2,5,'user',6,2,default,1);
insert into tblActivitybook values(4,2,7,'user',33,2,default,1);
insert into tblActivitybook values(5,2,8,'user',34,2,default,1);
insert into tblActivitybook values(6,2,11,'user',35,2,default,1);
...
insert into tblActivityReview values( 3,3,'user',4,'즐거운 활동이였습니다~',default);
insert into tblActivityReview values( 4,4,'user',2,'너무 재미있었어요!',default);
insert into tblActivityReview values( 5,5,'user',3,'즐거운 즐거웠어요!',default);
insert into tblActivityReview values( 6,6,'user',1,'재미없었어요 ㅜ',default);
...
-- 액티비티 리뷰 답변
create or replace view vwActReply as
select 
a.name as name , 
a.pid as pid  ,
1 as category,
...
ar.regdate as regdate,  
arp.seq as reply,
arp.content as rcontent,
arp.regdate as rregdate
from tblActivityReview ar 
left outer join tblActivityReply arp on ar.seq = arp.rseq
inner join tblActivityBook ab on ar.rseq = ab.seq
inner join tblActivity a on a.seq = ab.aseq;

-- 홈 리뷰 답변
create or replace view vwHomeReply as
select 
h.name as name,
h.pid as pid, 
2 as category,
...
hr.regdate as regdate, 
hrp.seq as reply,
hrp.content as rcontent,
hrp.regdate as rregdate
from tblHomeReview hr 
left outer join tblHomeReply hrp on hr.seq  = hrp.rseq
inner join tblHomeBook hb on hb.seq = hr.hseq
inner join tblRoom r on hb.rseq = r.seq
inner join tblHome h on h.seq = r.hseq;


-- 랜터카 리뷰 답변

create or replace view vwCarReply as
select 
c.carname as name,
c.pid as pid, 
3 as category,
...
crp.seq as reply,
crp.content as rcontent,
crp.regdate as rregdate
from tblCarReview cr 
left outer join tblCarReply crp on cr.seq  = crp.replyseq
inner join tblCarBook cb on cb.seq = cr.rseq
inner join tblCar c on c.seq = cb.cseq;

--통합 뷰
create or replace view vwReply as
select a.* from vwActReply a union select h.* from vwHomeReply h union select c.* from vwCarReply c; 

-- 통합뷰 ( 답변안된 값들 우선 출력 + 최근날짜 우선 출력)
select ROWNUM as num , r.* from (select * from vwReply  order by reply NULLS FIRST,regdate desc) r where ROWNUM between 1 and 6 and pid like 'partner';
...
job_name    => 'homebooklist_update',
job_type    => 'PLSQL_BLOCK',
job_action  => 'begin 
                update tblHomeBook set bsseq = 3 where enddate + 1 <= sysdate and bsseq = 2;
                update tblCarBook set bsseq = 3 where enddate + 1 <= sysdate and bsseq = 2;
                commit; 
                end;',
start_date  => TRUNC(SYSDATE)+1 + 0/24,
end_date    => NULL,
enabled => true,
