...
select * from tblRoom order by seq desc;
select * from tblHome order by seq desc;
select * from tblOptionList order by seq desc;
select * from tblRoomPic order by seq desc;
delete from tblHome where seq = 13;
delete from tblRoom where seq = 15;
delete from tblOptionList where seq = 15;
delete from tblRoomPic where seq = 4;
delete from tblRoomPic where seq = 5;
delete from tblRoomPic where seq = 6;
commit;
-- tblUser
insert into tblRoom values ((select max(seq)+1 from tblRoom), (select max(seq) from tblHome), 1, ?, ?, ?, 'n');
...
select * from tblCarType ct inner join tblCar c on c.ctseq = ct.seq inner join tblFuel f on c.fseq = f.seq inner join tblHeadcount hc on hc.seq = c.hseq left outer join tblCarBook cb on cb.cseq = c.seq left outer join tblCarReview cr on cr.rseq = cb.seq;
select * from TBLCARNAME;
select * from tblCar;
-- review view
